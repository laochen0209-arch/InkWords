  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        toast.title || toast.description ? "min-w-[280px] max-w-[320px]" : ""
      )}
      style={{
        // 墨迹晕染效果
        boxShadow: `
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        {toast.title && (
          <p className="text-sm font-medium text-[#FDFBF7] mb-1">
            {toast.title}
          </p>
        )}
        {toast.description && (
        ...
        {!toast.title && !toast.description && (
          <p className="text-sm font-serif text-[#FDFBF7] tracking-wide">
            {toast.message}
          </p>
        )}
      </div>
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToast = useCallback((message: string | { title: string; description: string }, type: ToastType) => {
    const id = `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`
    
    if (typeof message === 'string') {
      setToasts((prev) => [...prev, { id, message, type }])
    } else {
      setToasts((prev) => [...prev, { id, message: '', title: message.title, description: message.description, type }])
    }
  }, [])
