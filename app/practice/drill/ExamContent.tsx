 ...
 * @description 考试练习内容组件 - 包含所有客户端逻辑
...
export default function ExamContent() {
  ...
  if (loading) return (
    ...
    >
      ...
      <p className="ml-2 text-slate-600 font-medium">正在加载练习...</p>
    </div>
  );
  ...
  if (!exam) return (
    ...
    >
      ...
      <h2 className="text-xl font-bold text-slate-800">暂无 {currentType} 练习</h2>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-48">
              {exams.map((exam) => (
                ...
                >
                  {exam}
                  {currentType === exam && <Check className="w-4 h-4 text-amber-600" />}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <div className="grid gap-8 lg:grid-cols-12 items-start relative">
        ...
        <div className="lg:col-span-7 space-y-6 pb-20">
                    ...
                    {isMCQ ? (
                      ...
                      >
                        {q.options.map((opt: string, i: number) => {
                          ...
                          return (
                            <Label key={i} className={cn("flex items-center w-full p-4 rounded-lg border cursor-pointer transition-all", isSelected ? "border-amber-600 bg-amber-50 ring-1 ring-amber-600" : "border-slate-200 hover:bg-slate-50", isSubmitted && opt.startsWith(correctTag) && "border-green-500 bg-green-100")}>
                              <RadioGroupItem value={opt} id={`q${index}-${i}`} className="sr-only" />
                              <div className={cn("mr-3 h-4 w-4 rounded-full border", isSelected ? "border-amber-600 bg-amber-600" : "border-slate-300")} />
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea disabled={isSubmitted} value={userAnswer || ''} onChange={(e) => setUserAnswers(prev => ({...prev, [index]: e.target.value}))} placeholder="Type your answer here..." className="min-h-[80px] text-base border-slate-300 focus:border-amber-500 bg-white resize-none"/>
                      </div>
                    )}
                    {isSubmitted && (
          ...
          <div className="sticky bottom-0 p-4 bg-white/90 backdrop-blur border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] rounded-t-xl flex justify-end z-40 mt-8">
            {!isSubmitted ? (
              <Button onClick={handleSubmit} size="lg" className="w-full sm:w-auto bg-amber-600 hover:bg-amber-700 text-lg shadow-amber-200 shadow-lg">Submit Answers</Button>
            ) : (
              <Button onClick={() => window.location.reload()} size="lg" variant="outline" className="w-full sm:w-auto border-slate-300">Next Drill <ArrowLeft className="ml-2 h-4 w-4 rotate-180"/></Button>
            )}
          </div>
        </div>
      </div>
