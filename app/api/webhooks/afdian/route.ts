  ...
  try {
    ...
    if (!afdianUserId || !afdianToken) {
      ...
      return NextResponse.json(
        { ec: 400, em: 'Missing configuration' },
        { status: 400 }
      )
    }
    ...
    // 注意：爱发电的 Webhook 数据结构可能因版本不同而有所变化
    ...
    // ============================================================================
    // 7. 返回爱发电要求的响应格式
    // 必须返回 { "ec": 200, "em": "success" }，否则爱发电会无限重发
    // ============================================================================
    return NextResponse.json({ ec: 200, em: 'success' })

  } catch (error: any) {
    console.error('[Afdian Webhook] 处理过程出错:', error)
    // 即使出错也要返回成功，避免爱发电无限重发
    return NextResponse.json({ ec: 200, em: 'success' })
  }
}
